...
def preprocess_games_df(games_df):
  ...
  games_superset = pd.concat([games_df, existing_games]).drop_duplicates(subset=["GAME_ID"]).sort_values(by="GAME_DATE_EST")
  games_superset["GAME_DATE_EST"] = pd.to_datetime(games_superset["GAME_DATE_EST"])
...
def _calculate_b2bs(games_df, games_superset):
  games_df[["HOME_TEAM_B2B", "AWAY_TEAM_B2B"]] = games_df.apply(lambda game: _get_b2bs(game, games_superset), axis=1)
  return games_df
...
def _get_b2bs(game, games_superset):
  ...
  home_b2b = len(games_superset.loc[(games_superset['GAME_DATE_EST'] == date) & ((games_superset['HOME_TEAM_ID'] == home_team) | (games_superset['AWAY_TEAM_ID'] == home_team))])
  away_b2b = len(games_superset.loc[(games_superset['GAME_DATE_EST'] == date) & ((games_superset['HOME_TEAM_ID'] == away_team) | (games_superset['AWAY_TEAM_ID'] == away_team))])
...
def _get_game_lookback_data(game, games_superset):
  ...
  home_last_10_win_pct = _get_last_n_win_pct(home_team_id, 10, games_superset)
  ...
  away_last_10_win_pct = _get_last_n_win_pct(away_team_id, 10, games_superset)
